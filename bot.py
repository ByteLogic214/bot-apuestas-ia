...
def obtener_estadisticas_equipo(nombre_equipo):
    ...
    url = f"https://api-sports.io{nombre_equipo}"
    try:
        res = requests.get(url, headers=headers).json()
        if res.get("response"):
            team_id = res["response"][0]["team"]["id"]
            # Consultamos últimos partidos o rendimiento general
            url_stats = f"https://api-sports.io{team_id}&league=39"
...
def consultar_ia_rentabilidad(partido, cuotas, stats_local, stats_visita):
    ...
    try:
        response = requests.get(url, json=payload) if hasattr(requests, 'post') else requests.post(url, json=payload)
